...
$specs = @(
  ...
  @{ name='crypto-live';   exe=$bot;    args=@('--env','prod','live-trade','--dollars','2','--max-dollars','2','--real-money','--yes');
     match='kalshi-bot\.exe" --env prod live-trade'; stop=(Join-Path $state 'STOP'); state=(Join-Path $state 'live_loop.json') },
