...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* sports desk: a separate loop (kalshi-sports) with its own files, shown on its own tab */
  .sports-head { display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; margin: 0 0 14px; }
  .sports-head .who { display: flex; flex-direction: column; gap: 2px; }
  .sports-head .who b { font-size: 15px; }
  .sports-head .who .note { font-size: 12px; }
  .sports-head .controls { display: flex; gap: 8px; flex-wrap: wrap; align-items: center; }
  .sports .card { border-left: 3px solid var(--gold); }
  .sports .tile { grid-column: span 12; grid-template-columns: repeat(6, 1fr); }
  .sports .tile .value { font-size: 22px; }
  .sports .wide { grid-column: span 12; }
  .sports .ev-list { max-height: 280px; }
...
<main>
  ...
  <nav class="tabs">
    ...
    <button class="tab" data-view="sports" onclick="showView('sports')">Sports<span class="n" id="nsports"></span></button>
  </nav>
  ...
  <section class="view sports" id="view-sports">
    <div class="sports-head">
      <div class="who">
        <b id="sp-title">Sports desk · kalshi-sports</b>
        <span class="note" id="sp-cfg">separate loop, separate files: sports_*.json, sports_data.sqlite, SPORTS_STOP</span>
      </div>
      <div class="controls">
        ...
        <button class="ghost" onclick="spost('clear-stop')">Clear sports stop</button>
      </div>
    </div>
    ...
    <div class="grid">
      <section class="tile">
        ...
        <div class="card"><div class="label">Last tick</div><div class="value" id="sp-tick">–</div><div class="delta" id="sp-risk"></div></div>
      </section>
      <div class="card wide">
...
async function refreshSports() {
  ...
  $('nsports').textContent = s.open ? s.open : '';
  $('sp-title').textContent = 'Sports desk · kalshi-sports' + (live ? ' · real money' : mode === 'paper' ? ' · paper' : mode === 'dryrun' ? ' · dry run' : '');
  ...
  $('sp-events').innerHTML = evs.length ? evs.map(e =>
    `<div class="ev ${e.level || 'info'}"><span class="t">${tm(e.ts)}</span><span class="lvl"></span><span class="src">${esc(e.source || '')}</span><span class="x">${esc(String(e.text || ''))}</span></div>`
...
refreshSports(); setInterval(refreshSports, 3000);
</script>
</body>
</html>
"""
